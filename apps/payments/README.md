...
2. **PayPal.** В Developer Dashboard создайте sandbox-приложение, скопируйте Client ID / Secret. Для webhook нужен публичный URL (ngrok) на `https://<host>/webhooks/paypal`. `PAYPAL_WEBHOOK_ID` — id webhook-подписки. Оплата — sandbox buyer.
