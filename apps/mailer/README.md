...
Асинхронный сервис писем. Слушает очередь RabbitMQ `users.events`, SMTP — nodemailer. HTTP только внутренний health на `MAILER_HOST`:`MAILER_PORT` (по умолчанию `127.0.0.1:3001`). gRPC, GraphQL и БД отсутствуют. Публичные API на `0.0.0.0` не биндить — по умолчанию localhost.
...
- `user.created` — welcome-письмо («Вы зарегистрировались на платформе»)
- `user.authenticated` — игнорируется (общая очередь с `users`)
